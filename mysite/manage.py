...
Il permet d'exécuter diverses tâches administratives et de gestion, telles que le démarrage d'un serveur de développement,
la création de migrations, l'application de migrations, et l'exécution de tests.

Il agit comme une interface de ligne de commande pour les fonctionnalités offertes par le framework Django,
...
def main():
    ...
    Cette fonction configure l'environnement du projet en définissant le module de paramètres Django par défaut
    et délègue ensuite l'exécution à Django via `execute_from_command_line`. Elle attrape et relève également
